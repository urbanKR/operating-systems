...
public class PageClass {
	...
	public static int WorkingSetPage(int arr[], int frameNr) {
		//		int workingSet[] = new int[frameNr];
		//		for(int j=0; j<frameNr; j++) {
		//				workingSet[j] = 0;
		//		}
		//		int pageFNr = 0;
		...
		//			if(i < arr.length && i > 0)
		//			{
		//				if(!isInArr(arr[i-1], workingSet))
		//				{
		//					pageFNr++;
		//				}
		//				else if(unicalOcc(workingSet) >= frameNr)
		//				{
		//					pageFNr++;
		//				}
		//			}
		...
		int curr = 0;
		for(int i=0; i<arr.length; i ++) {
			if(!isInArr(arr[i], workingSet))
			{
				pageFNr++;
				workingSet[curr] = arr[i];
			}
			else if(unicalOcc(workingSet) >= frameNr)
			{
				pageFNr++;
			}
			if(curr == frameNr - 1) curr = 0;
		}
		return pageFNr;
	}

	public static int unicalOcc(int arr[]) {
		int unicalOcc = 0;
		sortAsc(arr);
		int x = arr[0];
		for(int i=0; i<arr.length; i++) {
